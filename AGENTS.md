...
```
CLAUDE.md                        <- Project context, remote agent URLs, core rules
AGENTS.md                        <- This file
README.md                        <- Human-facing repo overview and setup guide
.claude/
  README.md                      <- Claude Code operational reference
...
The core rules in CLAUDE.md apply to all agents. Changing them has
...
### When a task produces code (standard workflow)
The full sequence always ends with a PR. Claude Code should:
1. Create a feature branch before any file changes: git checkout -b feat/<description>
2. Let Developer, Reviewer, and QA agents do their work on that branch
3. When QA passes, DevOps agent opens a PR using the template in devops.md
4. Stop there. Do not merge. Notify the user the PR is ready for review.
...
```bash
git log --oneline .claude/agents/<agent>.md
